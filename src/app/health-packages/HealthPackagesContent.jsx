...
export default function HealthPackagesContent() {
  const { t } = useLanguage()
  const [booking, setBooking] = useState(null)
  const pathname = usePathname()
  const router = useRouter()

  // The home pickup form lives at its own URL so it can be shared and linked to.
  const pickupOpen = pathname === '/health-packages/book-home-pickup'
  const closePickup = () => router.push('/health-packages', { scroll: false })
  ...
  return (
    <div className="antialiased bg-slate-50 text-gray-800 min-h-screen flex flex-col">
      ...
      <main className="max-w-[1920px] mx-auto px-4 lg:px-16 py-12 w-full flex-1">
        ...
        <div className="max-w-4xl mx-auto mb-12">
          <div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-5 sm:p-6 flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
            ...
            <Link href="/health-packages/book-home-pickup" scroll={false}
      ...
      {pickupOpen && <HomePickupModal onClose={closePickup} />}
